...
class CNN(nn.Module):
    def __init__(self, FM, NC, Classes, HSIOnly, patchsize):#(16,144,15,False)
        ...
        self.position_embeddings = nn.Parameter(torch.zeros(1, FM*2 + 1, FM*4))#(1, 33, 64)
        self.dropout = nn.Dropout(0.1)
        self.FM = FM
        self.token_wA = nn.Parameter(torch.empty(1, FM*2, 64),
                                     requires_grad=True)  # Tokenization parameters (1, 32, 64)
        torch.nn.init.xavier_normal_(self.token_wA)
        self.token_wV = nn.Parameter(torch.empty(1, 64, 64),
                                     requires_grad=True)  # Tokenization parameters (1, 64, 64)
        torch.nn.init.xavier_normal_(self.token_wV)

    def forward(self, x1):#([2, 144, 121])
        x1 = x1.reshape(x1.shape[0],-1,self.patchsize,self.patchsize)#([2, 144, 11, 11])
        x1 = x1.unsqueeze(1)#([2, 1, 144, 11, 11])
        #Conv3D
        x1 = self.conv5(x1)#([2, 8, 136, 11, 11])           
        x1 = x1.reshape(x1.shape[0],-1,self.patchsize,self.patchsize)#([2, 1088, 11, 11])
        #HetConv2D
        x1 = self.conv6(x1)#([2, 64, 11, 11])
        #补丁标记
        cls_tokens = self.cls_token.expand(x1.shape[0], -1, -1)#([2, 1, 64])
        # Xflat
        x1 = x1.flatten(2)#([2, 64, 121])
        x1 = x1.transpose(-1, -2)#([2, 121, 64])
        # Xwa  
        wa = self.token_wA.expand(x1.shape[0],-1,-1)#([2, 32, 64])
        wa = rearrange(wa, 'b h w -> b w h')  # Transpose ([2, 64, 32])
        A = torch.einsum('bij,bjk->bik', x1, wa)#([2, 121, 32])
        A = rearrange(A, 'b h w -> b w h')  # Transpose ([2, 32, 121])
        A = A.softmax(dim=-1)#([2, 32, 121])
        # Xwb
        wv = self.token_wV.expand(x1.shape[0],-1,-1)# ([2, 64, 64])
        VV = torch.einsum('bij,bjk->bik', x1, wv)#([2, 121, 64])
        # Xpatch
        T = torch.einsum('bij,bjk->bik', A, VV)#([2, 32, 64])
        x = torch.cat((cls_tokens, T), dim = 1) #[b,n+1,dim] ([2, 33, 64])
        embeddings = x + self.position_embeddings#([2, 33, 64])
        embeddings = self.dropout(embeddings)#([2, 33, 64])
        x = x.reshape(x.shape[0],x.shape[1],int(math.sqrt(self.FM*4)),int(math.sqrt(self.FM*4)))#([2, 33, 8, 8])
        x = self.ca(x)#([2, 64])
        x = x.reshape(x.shape[0],-1)#([2, 64])
        x = self.out3(x)
        return x
...
if __name__ == '__main__':
    ...
    input = torch.randn(64, 102, 13, 13)
    y = model(input)
    print(y.size())


# patchsize = 13
# cnn = CNN(16, 102, 8, False)
# # cnn = cnn.cuda()
# summary(cnn, (102,169), device ='cpu')
# print('运行成功')
